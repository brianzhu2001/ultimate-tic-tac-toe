...
module MonteCarlo : Player = struct
    ...
    (**
        ...
        @param board current macro board state
        @param v move position being made, (locmacro, locmicro)
        @param acc accumulator of possible states
    *)
    let rec getNextPosStates (board:board) (v:int*int) (acc:state list): state list = 
        if (board.next_pos = -1) then (
          if snd v = 10 then acc else
          try 
            let newBoard = Macro.update board Machine (fst v) (snd v) in
                getNextPosStates 
                    board 
                    (getNextPerm v)
                    ({board = newBoard; move = v; winEst = 0}::acc)
          with | InvalidLoc -> getNextPosStates board (getNextPerm v) acc
        )
        else
          if snd v = 10 then acc else
            try
              let newBoard = Macro.update board Machine board.next_pos (snd v) in
                getNextPosStates board (0,snd v + 1)
                    ({board = newBoard; move = (-1,snd v) ;winEst = 0}::acc)
    ...
    let rec makeRandomMoves (board:board) (player:value)= 
      ...
      try
        ...
        | Machine -> 1
        | Human -> (-1)
        | Blank -> let rE = List.nth moves (Random.int (List.length moves)) in 
          makeRandomMoves 
              (Macro.update board (player) (fst rE) (snd rE))
              (oppPlayer player)
      with | InvalidLoc -> makeRandomMoves board player
        
    let rec getWinEst (board:board) (diff:int): int = 
      let numGames = diff in 
      let rec estimate (num:int) (acc:int) =
          if num = 0 then acc 
          else estimate (num-1) (makeRandomMoves board Machine + acc) in
      estimate numGames 0
